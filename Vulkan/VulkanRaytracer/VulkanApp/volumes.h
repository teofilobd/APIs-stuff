...
bool constant_medium::hit(const ray& r, float tmin, float tmax, hit_record& rec) const
{
	...
	if (boundary->hit(r, -FLT_MAX, FLT_MAX, rec1))
	{
		if (boundary->hit(r, rec1.t + 0.0001, FLT_MAX, rec2))
		{
			...
			float distance_inside_boundary = (rec2.t - rec1.t) * glm::length(r.direction());
			...
			if (hit_distance < distance_inside_boundary)
			{
				...
				rec.t = rec1.t + hit_distance / glm::length(r.direction());
				if (db)
				{
				...
				if (db)
				{
					std::cerr << "rec.p =" << rec.p.x << "," << rec.p.y << "," << rec.p.z << "\n";
				}
